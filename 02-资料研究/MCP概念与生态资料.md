...
| 来源 | 链接 / 位置 | 可用内容 | 使用边界 |
|---|---|---|---|
| MCP 中文站《一文读懂 MCP 协议：大模型 AI-Agent 的 USB-C 接口》 | https://mcpcn.com/blog/understanding-mcp-protocol/ | 拓展坞架构图、USB-C 类比 | 用户已说明已取得原作者授权；`A-07` 直接使用拓展坞原图。网页标注主笔 bexzhang、修订 osli，并注明来源“腾讯云智慧传媒”。授权说明存档，不在页面显示脚注。 |
...
- 页面实际图示：`03-素材库/05-图表与图示/A-07-AI设备拓展坞架构图-白底课件版.png`、`A-07-MCP客户端-服务器架构图-高清办公版.png`；前者按已授权原图重绘为纯白底，以匹配课件图卡；后者参照书中图 6-2 的关系与画风生成。
- 保留书中派生参考图：`03-素材库/05-图表与图示/A-07-MCP客户端-服务器架构图-书中派生.png`。
- 课件运行副本：`04-网页版PPT/课件源代码/assets/a07-mcp-usbc-hub-white.png`、`a07-mcp-client-server-workbuddy.png`。
