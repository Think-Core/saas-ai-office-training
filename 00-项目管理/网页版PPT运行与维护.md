...
- 当前 `A-xx` 与实际页码一致，因此 `?page=10` 对应 Subagent 概念页 `A-10`，`?page=20` 对应 WorkBuddy 能力组合页 `A-20`，`?page=23` 对应金融能力入口页 `A-23`，`?page=24` 对应 FAQ 页 `A-24`，`?page=33` 对应第四部分章节封面 `A-33`；当前课件共 33 页。
...
- 导出 HTML 应生成可单独打开的单文件；本地样式表、脚本、图片、视频、字体和常见音频都由导出器自动内嵌，不需手工复制配套目录；任一本地 CSS/JS 读取失败时导出会中止，页面已定义的翻页动画、元素/卡片时序和重播按钮也应保留。
- HTML 导出保存的是当前已完成页面的快照；导出文件重新打开时只启动导航、编辑、主题与动画运行时，不再重复执行首次建页、插页或旧页码迁移脚本。
