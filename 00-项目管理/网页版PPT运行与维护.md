...
- 当前 `A-xx` 与实际页码一致，因此 `?page=10` 对应 Subagent 概念页 `A-10`，`?page=13` 对应 `A-13`，`?page=20` 对应 WorkBuddy 能力组合页 `A-20`，`?page=24` 对应 `A-24`，`?page=32` 对应 `A-32`；当前课件共 32 页。
...
- 导出 HTML 应生成可单独打开的单文件；本地 `assets/` 样式表、脚本和媒体都由导出器自动内嵌，不需手工复制配套目录；页面已定义的翻页动画、元素/卡片时序和重播按钮也应保留。
